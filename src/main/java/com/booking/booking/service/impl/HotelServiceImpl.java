...
public class HotelServiceImpl implements HotelService {
    ...
    public Optional<Hotel> getHotelById(Long id) {
        return Optional.ofNullable(hotelRepository.findByIdAndIsDeletedFalse(id)
    ...
    public void deleteHotelsPermanently(List<Long> ids) throws ResourceNotFoundException {
        ...
        java.util.Set<Long> valid = list.stream().map(Hotel::getId).collect(java.util.stream.Collectors.toSet());
        List<Long> invalid = ids.stream().filter(id -> !valid.contains(id)).toList();
        if (!invalid.isEmpty()) {
